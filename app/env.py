...
class OpenAuditEnv:
    ...
    def step(self, action: AuditAction) -> tuple:
        ...
        if action.pillar != self.current_pillar:
            self.step_number += 1
            return self._get_observation(), 0.21, False, {"error": f"Wrong pillar: {action.pillar}"}

        # Grade the action
        reward_obj = self._grade_action(action)
        reward_value = round(min(0.99, max(0.01, float(reward_obj.value))), 3)
        ...
        return self._get_observation(), reward_value, self.completed, {"flaws_found": self.flaws_found_count}
