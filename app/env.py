...
"""
OpenAudit Environment - Using Official OpenEnv Base Class
"""
import json
import uuid
...
class OpenAuditEnv(Environment):
    """OpenAudit Environment - Inherits from official Environment base class"""
    ...
    def reset(self, task_id: str = None) -> AuditObservation:
        ...
        # Load artifact based on pillar with rich content
        if self.current_pillar == "model_card":
            ...
            content = json.dumps({"sample_rows": dataset[:20], "total_rows": len(dataset), "columns": list(dataset[0].keys()) if dataset else []}, indent=2)
            metadata = self.current_artifact.get("metadata", {})
            total_flaws = len([f for f in self.current_artifact.get("ground_truth_flaws", []) if f.get("type") == "null_values"])
            instructions = "Find null values in the dataset columns. Examine the sample data above."
            ...
            instructions = "Identify sparse reward issues. Analyze the reward function and trajectory."
        else:  # tool_tester
    ...
    def _get_observation(self) -> AuditObservation:
        """Build current observation with rich content"""
        total_flaws = self._get_total_flaws()
        content = ""
        metadata = {}
    ...
    def _get_total_flaws(self) -> int:
        """Count total flaws in current artifact"""
    ...
    def _get_instructions(self, task_id: str) -> str:
        ...
        instructions = {
            ...
            "model_card_medium": "Check for license conflicts. The parent model may have a different license.",
            "model_card_hard": "Verify benchmark claims. Look for discrepancies between claimed and actual scores.",
            "dataset_qc_easy": "Find null values in the dataset columns. Examine the sample data.",
            "dataset_qc_medium": "Detect duplicate rows in the dataset.",
            "dataset_qc_hard": "Identify train/test leakage in the dataset splits.",
            "rl_reward_easy": "Flag sparse reward patterns in the trajectory.",
            "rl_reward_medium": "Detect reward hacking behavior.",
            "rl_reward_hard": "Identify broken verifiers that never penalize.",
            "tool_tester_easy": "Find code quality issues: missing docstrings, type hints.",
            "tool_tester_medium": "Detect silent failure patterns (exception swallowing).",
            "tool_tester_hard": "Identify dangerous adversarial chains in tools.",
        }
